...
//------------------------------- PRIVADAS -----------------------------------//
Punto3D Camara2::generarPuntoAleatorioEnPixel(mt19937 gen, Pixel pixel){
...
void Camara2::crearCuadriculaPixeles(){
...
void Camara2::calcularLuzDirecta(Rayo rayo, Pixel p, Punto3D x, Primitiva* primitiva, Direccion normal){
    float valor = 0;
    Punto3D c;
    RGB potencia;
    float pot;
...
//------------------------------ CONSTRUCTORES -------------------------------//
Camara2::Camara2(Punto3D origen_val, Direccion u_val, Direccion l_val,
...
void Camara2::anyadirPrimitiva(Primitiva* primitiva){
    escena.anyadirPrimitiva(primitiva);
}

void Camara2::asignarEscena(Escena e){
    escena = e;
}

void Camara2::lanzarRayos(int rayosPorPixel){
...
ImagenPPM Camara2::crearImagenPPM(){
    float valorMax = 0;
    float v;
    vector<RGB> pixeles;
...
//-------------------------------- IMPRESION ---------------------------------//
ostream& operator<<(ostream& os, const Camara2& c) {
    os << "Camara2: (Origen: "/* << c.origen << ", Up: " << c.u << ", Left: " << c.l << ", Forward: " << c.f << ", tamPlanoImagen: " << c.tamPlanoImagen[0] << "x" << c.tamPlanoImagen[1]*/ << ")";
    return os;
}
...
vector<Pixel> Camara2::generarPixeles() {
